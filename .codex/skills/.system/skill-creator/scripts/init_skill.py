...
TEMPLATE_PATH = Path(__file__).resolve().parent.parent / "assets" / "skill-md-template.md"
...
def load_skill_template(skill_name, skill_title):
    """Load canonical SKILL.md template when available; fallback to bundled template."""
    if TEMPLATE_PATH.exists():
        text = TEMPLATE_PATH.read_text()
        return (
            text.replace("<skill-name>", skill_name)
...
def init_skill(skill_name, path, resources, include_examples, interface_overrides):
    ...
    skill_content = load_skill_template(skill_name=skill_name, skill_title=skill_title)
