...
```bash
DEV_BROWSER_DIR="${CODEX_HOME:-$HOME/.codex}/skills/dev-browser"
...
**Verify server health before running scripts:**

```bash
curl -fsS http://localhost:9222/ | rg wsEndpoint
```
...
**Packaging note:** `node_modules/`, `profiles/`, and `tmp/` are runtime artifacts. Do not treat them as canonical skill content.
