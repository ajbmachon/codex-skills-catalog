...
**Include when assumptions were corrected during the interview (either user's or Codex's).**
...
- Codex assumptions that investigation corrected
...
|---------------------|-------------|----------------------|-------------------------|
| [What was assumed] | User/Codex | [How we learned otherwise] | [What's actually true] |
| API X supports feature Y | User | Codex researched docs | API X requires Z workaround for Y |
| Common pattern is A | Codex | Codebase investigation | This project uses pattern B instead |
| Library handles edge case | User | Testing during interview | Library throws, need custom handling |
| Deprecated in v3 | Codex | Checked current docs | Still supported, deprecation reverted |
...
This isn't just about catching user mistakes. Document when Codex was wrong too:

```
| JWT tokens expire in 1 hour | Codex | User showed auth config | Configured for 24 hours in this app |
```
