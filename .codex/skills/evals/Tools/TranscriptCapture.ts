...
/**
 * Parse a JSONL session transcript into the internal format.
 */
...
export const parseClaudeCodeTranscript = parseJsonlTranscript;
...
if (import.meta.main) {
  ...
  if (command === 'parse' && args[0]) {
    ...
    const transcript = parseJsonlTranscript(content, 'test-task', 'trial-1');
